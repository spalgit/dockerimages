"""
Evaluate PTR v2 predictions: reverse-transform PTR -> pEC50 and score
against test_phase1.csv ground truth (253 compounds).

PTR v2 parameters: threshold=3.5, std=1.00
Reverse transform: pEC50_pred = norm.ppf(ptr_pred, 3.5, 1.00)
...
Usage:
    ...
    # Without LGBM gate:
    python ~/dockerimages/QSARTuna/PXR/evaluate_ptr_predictions_v2.py \
        --pred_csv  <model_output_dir>/test_prediction.csv \
        --output    ptr_v2_pec50_predictions.csv

    # With LGBM gate:
    python ~/dockerimages/QSARTuna/PXR/evaluate_ptr_predictions_v2.py \
        --pred_csv       <model_output_dir>/test_prediction.csv \
        --classifier_dir classifier_dir \
        --output         ptr_v2_pec50_predictions_gated.csv
"""
...
# Gate parameters (same as post_process_with_classifier.py)
GATE_LOW       = 0.30
GATE_HIGH      = 0.70
INACTIVE_MEDIAN = 3.0
BLEND_WEIGHT   = 0.60
...
def apply_lgbm_gate(df, classifier_dir):
    ...
    low   = p_active < GATE_LOW
    gated[low] = BLEND_WEIGHT * INACTIVE_MEDIAN + (1 - BLEND_WEIGHT) * df.loc[low, "pEC50_pred"]
    df["pEC50_pred_gated"] = gated

    print(f"\nLGBM gate: {low.sum()} compounds blended (P_active < {GATE_LOW}), "
...
def main():
    ...
    parser.add_argument("--pred_csv",       required=True,
                        help="test_prediction.csv from openadmet predict (PTR v2 space)")
    parser.add_argument("--output",         default="ptr_v2_pec50_predictions.csv")
    ...
    if args.classifier_dir:
        ...
        scorecard(df["pEC50"].values, df["pEC50_pred_gated"].values,
                  label="PTR v2 + ChemProp + LGBM gate")
