...
ChemProp PXR pEC50 — RDKit 2D descriptors + counter-assay weighting
                     + inverse-variance SE weighting.
Training set: f1_plus_htchem_train_minus_phase1.sdf  (4575 compounds; phase1
              test set removed from the original 4827-compound set)
Test set    : test_phase1.sdf                        (252 Phase 1 compounds
              with known pEC50 labels — used for held-out evaluation)

Weighting   : combined weight = counter_weight × se_weight, normalised to
              mean = 1.0.

  counter_weight : linear rescaling of counter pEC50 → [0.5, 2.0].
                   Compounds without counter data → 1.0 (neutral).

  se_weight      : inverse-variance  1 / SE²,  normalised to mean = 1.0.
                   Compounds without SE data  → 1.0 (neutral).

The combined weight reflects both assay selectivity (counter screen) and
measurement precision (DRC fitting + yield uncertainty for HTChem).
HTChem crudes naturally receive lower se_weight because their propagated
SE is larger than that of pure compounds.

Compare with chemprop_htchem_counter_plus_se_weight.py which uses the full
4827-compound training set and phase2 as the test set.

Usage:
    conda activate chemprop
    python /home/spal/dockerimages/QSARTuna/PXR/June_19/chemprop_htchem_counter_plus_se_weight_phase1test.py
"""
...
TRAIN_SDF = HERE / "f1_plus_htchem_train_minus_phase1.sdf"
TEST_SDF  = HERE / "test_phase1.sdf"

CV_RESULTS_PATH = HERE / "htchem_counter_se_phase1_cv_results.csv"
ENSEMBLE_DIR    = HERE / "htchem_counter_se_phase1_ensemble"
OUTPUT_PREDS    = HERE / "htchem_counter_se_phase1_test_predictions.csv"
KEPT_DESCS_PATH = HERE / "htchem_counter_se_phase1_kept_descriptors.txt"

# ── SD tag names ───────────────────────────────────────────────────────────────
TAG_TARGET  = "pEC50"
TAG_COUNTER = "pEC50_counter"
TAG_SE      = "pEC50_std.error"
...
def compute_counter_weights(counter_values: np.ndarray) -> np.ndarray:
    ...
    Compounds without counter data get NEUTRAL_WEIGHT."""
...
def compute_se_weights(se_values: np.ndarray) -> np.ndarray:
    """Inverse-variance weights: 1 / SE², normalised to mean = 1.0.
    Compounds without SE data get NEUTRAL_WEIGHT."""
    weights = np.full(len(se_values), NEUTRAL_WEIGHT, dtype=float)
    has_se  = ~np.isnan(se_values) & (se_values > 0)
    inv_var = 1.0 / (se_values[has_se] ** 2)
    inv_var_normalised      = inv_var / inv_var.mean()
    weights[has_se]         = inv_var_normalised
    return weights


def combine_weights(counter_w: np.ndarray, se_w: np.ndarray) -> np.ndarray:
    """Element-wise product, renormalised to mean = 1.0."""
    combined = counter_w * se_w
...
# ══════════════════════════════════════════════════════════════════════════════
print(f"\n{'='*70}")
print("HTChem F1 (phase1-excluded): 4575 compounds")
print("Test set : Phase 1 (252 compounds with known pEC50 labels)")
print("Weighting: counter-assay  ×  inverse SE² (combined, normalised)")
print(f"{'='*70}")
...
train_mols, train_names, train_smiles, train_tags = load_sdf(
    ...
    optional_tags=[TAG_COUNTER, TAG_SE],
)
train_targets  = train_tags[TAG_TARGET]
counter_values = train_tags[TAG_COUNTER]
se_values      = train_tags[TAG_SE]

counter_w    = compute_counter_weights(counter_values)
se_w         = compute_se_weights(se_values)
train_weights = combine_weights(counter_w, se_w)

n_counter = (~np.isnan(counter_values)).sum()
n_se      = (~np.isnan(se_values)).sum()
...
print(f"  SE available             : {n_se}/{len(train_mols)} "
      f"({100*n_se/len(train_mols):.1f}%)")
print(f"  SE range                 : {np.nanmin(se_values):.3f} – "
      f"{np.nanmax(se_values):.3f}  (mean {np.nanmean(se_values):.3f})")
print(f"  Counter weight range     : {counter_w.min():.3f} – {counter_w.max():.3f}")
print(f"  SE weight range          : {se_w.min():.3f} – {se_w.max():.3f}")
print(f"  Combined weight range    : {train_weights.min():.3f} – {train_weights.max():.3f}")
print(f"  pEC50 range              : {train_targets.min():.2f} – {train_targets.max():.2f} "
      f"(mean {train_targets.mean():.2f})")

print(f"\nLoading test SDF (Phase 1 with labels):\n  {TEST_SDF}")
...
feat = featurizers.SimpleMoleculeMolGraphFeaturizer()
test_dps    = make_datapoints(test_mols, test_targets,
...
# ══════════════════════════════════════════════════════════════════════════════
# Step 6 — Ensemble prediction and evaluation on Phase 1
# ══════════════════════════════════════════════════════════════════════════════
...
print(f"\n{'='*70}")
print("Ensemble complete — counter weighting × inverse SE² weighting")
print("Phase 1 held-out evaluation")
print(f"{'='*70}")
...
print("\nPerformance on Phase 1 test set (ensemble):")
report_metrics(test_targets, ensemble_preds, label="Phase1 ensemble")
