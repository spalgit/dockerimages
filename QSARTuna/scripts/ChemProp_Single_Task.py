...
#        log_transform_unit_conversion=6,
        training_dataset_file="~/dockerimages/QSARTuna/processed_CYP3A4_inhibition_train.csv",
        test_dataset_file="~/dockerimages/QSARTuna/processed_CYP3A4_inhibition_test.csv"
    ),
    descriptors=[
        SmilesFromFile.new()
    ],
    ...
    settings=OptimizationConfig.Settings(
        ...
        n_splits=2, # Increase it to 5
        n_replicates=3,
        n_trials=50,  # Increase to 100+ for production
        n_startup_trials=10, # Increase it to 20 for production
        random_seed=42,
        scoring="r2",
        n_jobs=-1,
...
#predicted_proba = model.predictor.predict_proba(smiles_col,with_aux=True)  # Returns DataFrame with ALL tasks!

pd.DataFrame(list(zip(smiles_col,expected,predicted_ksol)), columns=['SMILES', 'pIC50','pIC50_predicted']).to_csv(f"{dirname}/test_all_predictions.csv",index=False)
#pd.DataFrame(list(zip(smiles_col, expected,predicted_proba))).to_csv(f"{dirname}/test_all_predictions_proba.csv",index=False)


pd.DataFrame(list(zip(smiles_col, predicted_ksol)), columns=['SMILES','pIC50_predicted']).to_csv(f"{dirname}/test_all_predictions_1.csv",index=False)
