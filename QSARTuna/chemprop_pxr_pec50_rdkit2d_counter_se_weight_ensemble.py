...
ChemProp PXR pEC50 — RDKit 2D descriptors + counter-assay weighting
                      + pEC50 standard-error weighting + multi-seed ensemble.

Extends chemprop_pxr_pec50_rdkit2d_counter_weight_ensemble.py with a second
weighting factor derived from the measurement standard error (std_error column).

Weighting scheme:
  1. Counter-screen weight  (unchanged from the original script):
  ...
  2. Standard-error weight  (NEW):
       std_error INVERSELY mapped to [SE_MIN_WEIGHT, SE_MAX_WEIGHT]
         low std_error  → high weight  (precise measurement, trust it more)
         high std_error → low weight   (noisy measurement, down-weight)
         NaN            → SE_NEUTRAL_WEIGHT

  Final weight = counter_weight × se_weight
    Both factors are centred at 1.0 for their "neutral" values.
    Combined range: [MIN_WEIGHT × SE_MIN_WEIGHT, MAX_WEIGHT × SE_MAX_WEIGHT].

Usage:
    conda activate chemprop
    python ~/dockerimages/QSARTuna/chemprop_pxr_pec50_rdkit2d_counter_se_weight_ensemble.py
"""
...
CV_RESULTS_PATH  = Path.home() / "pxr_rdkit2d_cw_se_ensemble_cv_results.csv"
ENSEMBLE_DIR     = Path.home() / "pxr_cw_se_ensemble_models"
OUTPUT_PREDS     = Path.home() / "pxr_rdkit2d_cw_se_ensemble_test_predictions.csv"
KEPT_DESCS_PATH  = Path.home() / "pxr_rdkit2d_cw_se_ensemble_kept_descriptors.txt"
...
# Combined weight range: [0.5×0.5, 2.0×2.0] = [0.25, 4.0]
SE_MIN_WEIGHT     = 0.5
SE_MAX_WEIGHT     = 2.0
SE_NEUTRAL_WEIGHT = 1.0
...
def compute_se_weights(se_values: np.ndarray) -> np.ndarray:
    """Inverse linear map of std_error → [SE_MIN_WEIGHT, SE_MAX_WEIGHT].

    Low std_error → high weight (precise measurement).
    High std_error → low weight (noisy measurement).
    NaN → SE_NEUTRAL_WEIGHT.
    """
    weights = np.full(len(se_values), SE_NEUTRAL_WEIGHT, dtype=float)
    has_se  = ~np.isnan(se_values)
    vals    = se_values[has_se]
    se_min, se_max = vals.min(), vals.max()
    if se_max > se_min:
        weights[has_se] = SE_MIN_WEIGHT + (SE_MAX_WEIGHT - SE_MIN_WEIGHT) * (
            (se_max - vals) / (se_max - se_min)
        )
    # If all SE values are identical there is no information → leave at neutral.
    return weights


def combine_weights(counter_weights: np.ndarray, se_weights: np.ndarray) -> np.ndarray:
    """Multiply the two weight vectors element-wise."""
    return counter_weights * se_weights
...
se_values      = df_train[STD_ERROR_COL].values

counter_weights = compute_counter_weights(counter_values)
se_weights      = compute_se_weights(se_values)
train_weights   = combine_weights(counter_weights, se_weights)

n_with_counter = (~np.isnan(counter_values)).sum()
n_with_se      = (~np.isnan(se_values)).sum()
...
print(f"  Counter weight range     : "
      f"{counter_weights.min():.3f} – {counter_weights.max():.3f}  "
      f"(neutral={NEUTRAL_WEIGHT})")

print(f"\n  std_error available      : {n_with_se}/{len(df_train)} "
      f"({100*n_with_se/len(df_train):.1f}%)")
print(f"  std_error range          : "
      f"{np.nanmin(se_values):.4f} – {np.nanmax(se_values):.4f}  "
      f"(mean {np.nanmean(se_values):.4f})")
print(f"  SE weight range          : "
      f"{se_weights.min():.3f} – {se_weights.max():.3f}  "
      f"(neutral={SE_NEUTRAL_WEIGHT})")

print(f"\n  Combined weight range    : "
      f"{train_weights.min():.3f} – {train_weights.max():.3f}  "
      f"(neutral product={NEUTRAL_WEIGHT * SE_NEUTRAL_WEIGHT:.1f})")
