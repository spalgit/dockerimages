...
CSV input version using the error-smoothed training set.

Training data: train_cliff_error_weighted_median.csv (4124 compounds)
  - 16 high-error pair compounds removed
  - Activity-cliff series smoothed using experimental error rules:
      Series (3+): high-error compounds (pEC50_std.error/pEC50 > 5%) excluded from median
      Pairs: high-error compound removed; partner kept unchanged
      Pairs (both low/high error): mean used
  Note: SMILES augmentation is NOT applied here — RDKit 2D descriptors are
  molecule-level and invariant to SMILES ordering, so augmentation adds no value.

Weighting scheme (same as original SDF script):
    pEC50_counter INVERSELY mapped to [MIN_WEIGHT, MAX_WEIGHT]:
        low counter potency  → high weight  (selective compound, trust more)
        high counter potency → low weight   (promiscuous compound, down-weight)
        NaN                  → NEUTRAL_WEIGHT

Usage:
    conda activate chemprop
    python ~/dockerimages/QSARTuna/chemprop_pxr_pec50_rdkit2d_cw_ensemble_error_smoothed.py
"""
...
# ── Paths ──────────────────────────────────────────────────────────────────────
TRAIN_CSV = Path(
    "/home/spal/dockerimages/QSARTuna/PXR/train_cliff_error_weighted_median.csv"
)
TEST_SDF = Path(
    "/home/spal/dockerimages/QSARTuna/PXR/test_OpenADMET_Data_prepped.sdf"
)
CV_RESULTS_PATH = Path.home() / "pxr_rdkit2d_cw_ens_error_smoothed_cv_results.csv"
ENSEMBLE_DIR    = Path.home() / "pxr_ensemble_models_error_smoothed"
OUTPUT_PREDS    = Path.home() / "pxr_rdkit2d_cw_ens_error_smoothed_test_predictions.csv"
KEPT_DESCS_PATH = Path.home() / "pxr_rdkit2d_cw_ens_error_smoothed_kept_descriptors.txt"
...
def load_csv(csv_path: Path):
    """
    Load training CSV and return mols, names, smiles, pEC50, pEC50_counter arrays.
    Rows where SMILES cannot be parsed are dropped.
    """
    ...
    if skipped:
        print(f"  {skipped} rows skipped (unparseable SMILES)")
...
train_mols, train_names, train_smiles, train_targets, counter_values = load_csv(TRAIN_CSV)
