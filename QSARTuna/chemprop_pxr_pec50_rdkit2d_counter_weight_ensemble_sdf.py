...
SDF input version of chemprop_pxr_pec50_rdkit2d_counter_weight_ensemble.py.

Training SDF SD tags used:
    pEC50            — primary assay activity (regression target)
    pEC50_counter    — counter-screen potency (used for weighting; NaN → neutral weight)
    sample_weight    — pre-computed weight from CSV pipeline (not used here;
                       weights are recomputed from pEC50_counter below)

Weighting scheme (unchanged from original):
    pEC50_counter INVERSELY mapped to [MIN_WEIGHT, MAX_WEIGHT]:
        low counter potency  → high weight  (selective, trust it more)
        high counter potency → low weight   (promiscuous, down-weight)
        NaN                  → NEUTRAL_WEIGHT

Usage:
    conda activate chemprop
    python ~/dockerimages/QSARTuna/chemprop_pxr_pec50_rdkit2d_counter_weight_ensemble_sdf.py
"""
...
CV_RESULTS_PATH  = Path.home() / "pxr_rdkit2d_cw_ens_sdf_cv_results.csv"
ENSEMBLE_DIR     = Path.home() / "pxr_ensemble_models_sdf"
OUTPUT_PREDS     = Path.home() / "pxr_rdkit2d_cw_ens_sdf_test_predictions.csv"
KEPT_DESCS_PATH  = Path.home() / "pxr_rdkit2d_cw_ens_sdf_kept_descriptors.txt"
...
def load_sdf(sdf_path: Path, required_tags: list, optional_tags: list = None):
    """
    Load an SDF file and return (mols, names, tag_dict).

    Explicit Hs are removed after loading so that the heavy-atom graph seen by
    ChemProp and RDKit descriptors matches what MolFromSmiles would produce,
    while preserving formal charges and the correct protonation state set by MOE.

    Returns
    -------
    mols         : list[Mol]
    names        : list[str]
    tag_values   : dict[tag_name -> np.ndarray]  (float; NaN for missing)
    smiles_list  : list[str]  canonical SMILES regenerated from the loaded mol
    """
