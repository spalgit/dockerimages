...
# For single node installation follow this
...
#
# COMPUTE NODES
NodeName=$hostname CPUs=$CPUS Boards=$BOARDS Sockets=$SOCKETS CoresPerSocket=$CORES_PER_SOCKET ThreadsPerCore=$THREADS_PER_CORE RealMemory=$REALMEM Gres=gpu:$gpu_count
...
gpu_count=$(nvidia-smi --list-gpus | wc -l)
full_gpu_name=$(nvidia-smi --query-gpu=name --format=csv,noheader | head -n 1)

if echo "$full_gpu_name" | grep -q 'T4'; then
  gpu_type="t4"
elif echo "$full_gpu_name" | grep -q 'L4'; then
  gpu_type="l4"
else
  # fallback or extract another part of name if needed
  gpu_type=$(echo "$full_gpu_name" | awk '{print $NF}')  # last word by default
fi


sudo tee $GRES_CONF > /dev/null << EOF
$(for i in $(seq 0 $((gpu_count-1))); do echo "NodeName=$hostname Name=gpu Type=$gpu_type File=/dev/nvidia$i"; done)
EOF
