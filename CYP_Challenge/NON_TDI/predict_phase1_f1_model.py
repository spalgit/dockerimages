"""
Inference script: F1 ChemProp ensemble → Phase 1 test set

NOTE: The F1 training set includes the 253 phase=1 compounds, so this
evaluates in-sample fit (memorisation), not held-out generalisation.
Compare against predict_phase1_f2_model.py for a fair out-of-sample view.

Steps:
  1. Load test_phase1.sdf (already extracted by predict_phase1_f2_model.py;
     re-extracted here if missing)
  2. Re-derive descriptor column mask + StandardScaler from the F1 training set
  3. Load each saved f1_ensemble_models/model_seed*.pt and predict
  4. Report performance metrics against known pEC50 labels

Usage (inside the VM):
    conda activate chemprop
    python /home/spal/dockerimages/QSARTuna/PXR/June_14_ligprepped/predict_phase1_f1_model.py
"""
...
F1_TRAIN_SDF    = HERE / "train_set_AND_phase_one_results_4392_ligpreped_f_1.sdf"  # same file; phase=1 compounds are in training
ENSEMBLE_DIR    = HERE / "f1_ensemble_models"
TEST_PHASE1_SDF = HERE / "test_phase1.sdf"
OUTPUT_CSV      = HERE / "test_phase1_f1_predictions.csv"
...
# ══════════════════════════════════════════════════════════════════════════════
# Step 1 — Extract phase=1 compounds if test_phase1.sdf does not yet exist
# ══════════════════════════════════════════════════════════════════════════════
print(f"\n{'='*70}")
print("Step 1: Checking for test_phase1.sdf")
print(f"{'='*70}")

if not TEST_PHASE1_SDF.exists():
    print("  test_phase1.sdf not found — extracting from F1 SDF...")
    ...
    print(f"  Found existing {TEST_PHASE1_SDF} — skipping extraction")
...
print(f"  NOTE: these compounds were in the F1 training set — metrics reflect in-sample fit")

# ══════════════════════════════════════════════════════════════════════════════
# Step 3 — Load F1 training set and re-derive descriptor mask + scaler
# ══════════════════════════════════════════════════════════════════════════════
print(f"\n{'='*70}")
print("Step 3: Re-deriving descriptor selector and scaler from F1 training set")
print(f"{'='*70}")

train_suppl = Chem.SDMolSupplier(str(F1_TRAIN_SDF), removeHs=True)
train_mols  = [m for m in train_suppl if m is not None]
print(f"  Loaded {len(train_mols)} F1 training molecules")
...
final_xd_scaler.fit(x_d_train_raw)
...
# ══════════════════════════════════════════════════════════════════════════════
print(f"\n{'='*70}")
print("Step 5: Running ensemble inference (F1 models)")
print(f"{'='*70}")
...
# ══════════════════════════════════════════════════════════════════════════════
print(f"\n{'='*70}")
print("Step 6: Ensemble results (F1 — phase=1 compounds were in training set)")
print(f"{'='*70}")
...
if has_label.sum() > 1:
    print(f"\nPerformance vs. known pEC50 (n={has_label.sum()}) — IN-SAMPLE:")
    metrics = report_metrics(test_pec50[has_label], ensemble_preds[has_label],
                             label="F1 ensemble → Phase 1 (in-sample)")
