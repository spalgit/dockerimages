...
FILES = [
    "train_set_AND_phase_one_results_4392_ligpreped_f_1_n.sdf",
    "train_set_4139_ligpreped_f_2_n.sdf",
    "test_phase2_ligprepped_f_2_n.sdf",
]
...
def neutralize_nplus(mol):
    ...
    for atom in rw.GetAtoms():
        ...
        if h_neighbors:
            # Tag the atom: indices shift once the H atoms are removed, so the
            # re-scan below must identify these N by property, not by index.
            atom.SetBoolProp("_nplus_to_neutralize", True)
    ...
    # After removals, N indices may have shifted — re-scan using the tag so
    # only the N whose H we actually removed are neutralized.  Quaternary [n+]
    # in the same molecule must keep their charge; neutralizing them breaks the
    # ring (kekulization failure).
    for atom in rw.GetAtoms():
        if not atom.HasProp("_nplus_to_neutralize"):
            continue
        h_nbrs = [n for n in atom.GetNeighbors() if n.GetAtomicNum() == 1]
        if not h_nbrs:               # H was removed — safe to neutralize
            atom.SetFormalCharge(0)
        atom.ClearProp("_nplus_to_neutralize")
