...
# ── Censored non-hits (change 1) ───────────────────────────────────────────────
# Compounds screened but never carried to a DRC are currently invisible to the pIC50
# heads: build_primary_labels leaves them NaN and chemprop masks them out. For CYP2D6
# that discards ~2,800 compounds covering precisely the inactive range the test set is
# full of (training q10 = 4.13; the blinded set is ~73% screening non-hits).
#
# The fix is not a pseudo-pIC50 — that is the proxy arm, and its CYP2D6 calibration is
# r2 = 0.50. It is a *bound*: a confident non-hit satisfies "pIC50 <= bound", which is
# a true statement of unknown precision. lt_mask + BoundedMSE charge nothing for any
# prediction under the bound and only penalise calling an inactive compound potent.
CENSOR_NONHITS = True
# The bound is NOT the assay top. Among non-hits that did get a DRC, only 40% of CYP2D6
# and 52% of CYP2C9 came back below 4.305, so censoring there would be false more often
# than true. Take the bound from the observed non-hit DRC distribution instead: the
# CENSOR_QUANTILE-th percentile is a claim that holds for that fraction of non-hits.
# Fitted per isoform inside the training fold (see fit_censoring_bounds).
CENSOR_QUANTILE = 0.90
CENSOR_STRICT_LOG2FC = -0.5  # only censor clear non-inhibitors, not borderline misses
CENSOR_WEIGHT = 0.3  # compound weight for pure-censored rows, vs 1.0 for real DRC rows
CENSOR_MIN_NONHIT_DRC = 30  # below this, fall back to TOP_CONC_PIC50 for the bound
...
# Set to (ISOFORMS,) to recover the single joint model.
TASK_ARMS = (("CYP1A2", "CYP2C9", "CYP3A4"), ("CYP2D6",))

# ── Evaluation reweighting (change 3) ──────────────────────────────────────────
# The DRC-labelled set is not the test set. Compounds were promoted to a DRC because
# they were hits, so 80% of the CYP2D6 OOF set are hits against 27% of the screened
# population — and the blinded test set is dense (750 compounds x 4 isoforms), so it
# follows the population rate. Scoring OOF unweighted therefore measures the wrong
# distribution, which is why the alpha sweep saw nothing to gain at the weak end and why
# the scaffold hold-out mis-ranked architectures. Importance-weight each OOF compound so
# hits and non-hits appear in population proportion.
EVAL_REWEIGHT = True
...
# attach (fit_mpnn only builds callbacks when val_dset is not None) — the old
# max(CV_MAX_EPOCHS * 1.2, 60) trained a flat 60 epochs while CV put the optimum at 8-10.
# Take the budget from the folds instead, scaled up a little for the ~25% extra data a
# full-data refit sees. Set FINAL_EPOCHS_LEGACY = True to restore the fixed 60.
...
# ── Hyperparameters ────────────────────────────────────────────────────────────
# The previous grid was flat: mean_val_loss spanned 0.2275-0.2296 (<1%) and macro ST-RAE
# 0.734-0.755 across all four combos, so hyperparameters are not where the error is. With
# the arm split doubling the number of fits, re-running it costs 40 fits to re-discover a
# tie. Default to the previous winner (mp_depth 4, mp_hidden_dim 300) and spend the time
# on the label and evaluation changes instead. Set RUN_GRID_SEARCH = True to sweep again —
# worth doing once the label set has settled, since the old ranking was chosen on the
# unweighted objective and a single joint arm.
...
DEFAULT_PARAMS = dict(
    mp_depth=4, mp_hidden_dim=300, ffn_hidden_dim=600, ffn_n_layers=2, dropout=0.1
)
...
def st_rae_weighted(y_true, y_pred, lower, upper, w) -> float:
    """ST-RAE with per-compound importance weights.

    Same soft-threshold rule as the challenge scorer — zero cost inside
    ``[lower, upper]``, otherwise the distance to the nearest bound — but each compound
    contributes ``w`` times. The naive-predictor denominator is reweighted identically
    and its constant is the *weighted* mean, so 1.0 keeps meaning "no better than
    predicting the mean of the population being scored".
    """
    err = np.clip(y_pred - upper, 0, None) + np.clip(lower - y_pred, 0, None)
    mu = float(np.average(y_true, weights=w))
    base = np.clip(mu - upper, 0, None) + np.clip(lower - mu, 0, None)
    denom = float(np.sum(w * base))
    return float(np.sum(w * err) / denom) if denom > 0 else float("nan")


def population_eval_weights(df: pd.DataFrame) -> np.ndarray:
    """(n x 4) importance weights reweighting the DRC-labelled set to population hit rate.

    Compounds reached a DRC because they screened as hits, so the labelled set over-
    represents actives — by 2.9x on CYP2D6 and 2.5x on CYP1A2, but only 1.2x on CYP2C9.
    That ordering is the leaderboard ordering. Reweighting hits by
    ``p_pop / p_drc`` and non-hits by ``(1 - p_pop) / (1 - p_drc)`` makes an OOF score an
    estimate of test-set performance rather than of DRC-subset performance.

    Weights are normalised to mean 1 within each isoform, so ``n`` stays interpretable.
    """
    n = len(df)
    w = np.ones((n, N_ISO))
    if not EVAL_REWEIGHT:
        return w
    for j, iso in enumerate(ISOFORMS):
        ...
        screened = np.isfinite(df[f"{iso}_log2fc_estimate"].to_numpy(dtype=float))
        labelled = np.isfinite(pic50)
        if screened.sum() < 50 or labelled.sum() < 50:
            continue
        p_pop = float(is_hit[screened].mean())
        p_drc = float(is_hit[labelled].mean())
        if not (0 < p_pop < 1) or not (0 < p_drc < 1):
            continue
        wj = np.where(is_hit, p_pop / p_drc, (1 - p_pop) / (1 - p_drc))
        wj = np.where(labelled, wj, 1.0)
        m = labelled
        if m.sum():
            wj = wj / float(wj[m].mean())
        w[:, j] = wj
    return w


def score_per_isoform(
    truth: pd.DataFrame, preds: np.ndarray, label: str = "",
    eval_w: np.ndarray | None = None,
) -> dict:
    """ST-RAE / MAE / Spearman per isoform plus the macro average the board ranks on.

    ``eval_w`` supplies per-compound importance weights (see population_eval_weights).
    When it is None the unweighted challenge scorer is used, which is what the
    leaderboard applies to the dense test set and what earlier runs reported.
    """
    rows = []
    for j, iso in enumerate(ISOFORMS):
        ...
        if eval_w is None:
            score = st_rae(yv, pv, lo, hi)
            mae = float(np.mean(np.abs(yv - pv)))
        else:
            wv = eval_w[m, j]
            score = st_rae_weighted(yv, pv, lo, hi, wv)
            mae = float(np.average(np.abs(yv - pv), weights=wv))
        rows.append(
            dict(isoform=iso, n=int(m.sum()), st_rae=score, mae=mae, spearman=float(rho))
        )
...
def fit_censoring_bounds(train_df: pd.DataFrame, verbose: bool = False) -> dict:
    """Per-isoform upper bound for a confident non-hit, learned from the training fold.

    Among compounds that screened as non-hits *and* were nonetheless carried to a DRC,
    what pIC50 do they actually come back at? That distribution is the evidence for how
    strong a statement "non-hit" is, and it differs sharply by isoform:

        CYP3A4  median 2.97, 99% below the 4.305 assay top -> non-hit means inactive
        CYP1A2  median 3.81, 78% below
        CYP2C9  median 4.28, 52% below
        CYP2D6  median 4.35, 40% below                     -> non-hit means "not potent"

    Censoring every non-hit at TOP_CONC_PIC50 would therefore be a false claim for the
    majority of CYP2D6 compounds. Taking the CENSOR_QUANTILE-th percentile instead makes
    the bound true for that fraction of the non-hits we can actually check, per isoform.
    """
    bounds = {}
    for iso in ISOFORMS:
        pic50 = train_df[f"{iso}_pIC50_direct_inhibition"].to_numpy(dtype=float)
        is_hit = train_df[f"{iso}_is_hit"].to_numpy(dtype=bool)
        log2fc = train_df[f"{iso}_log2fc_estimate"].to_numpy(dtype=float)
        # Two traps here. First, build_compound_table fills is_hit with False for
        # compounds absent from the screen, so "not a hit" also catches 530 CYP3A4
        # compounds that were never screened at all — they carry real DRC potencies and
        # would drag the bound from 3.89 up to 5.03, above the assay top. Require a
        # finite log2fc so only genuinely screened compounds vote. Second, estimate the
        # bound on the same population it gets applied to (the strict non-hits), not on
        # all non-hits, or the bound describes a different set than the one it labels.
        eligible = (
            (~is_hit)
            & np.isfinite(pic50)
            & np.isfinite(log2fc)
            & (log2fc > CENSOR_STRICT_LOG2FC)
        )
        vals = pic50[eligible]
        if len(vals) < CENSOR_MIN_NONHIT_DRC:
            # CYP2D6 lands here every time, and it is a fact about the campaign rather
            # than a failure. All 1,198 of its screening hits were carried to a DRC, so
            # the only non-hits that got one were deliberate near-misses: all 295 sit in
            # log2fc [-1.00, -0.59], none above CENSOR_STRICT_LOG2FC. There is therefore
            # no direct evidence of what a *clear* CYP2D6 non-inhibitor assays at.
            #
            # Falling back to the assay top is the conservative reading. The compounds we
            # are labelling are weaker inhibitors than those observed near-misses (which
            # have median pIC50 4.35), so their true potencies should sit below that, and
            # for a censored upper bound erring high is the safe direction: too high
            # merely makes the constraint non-binding, too low teaches a wrong value.
            bounds[iso] = dict(bound=TOP_CONC_PIC50, n=len(vals), source="assay_top")
        else:
            bounds[iso] = dict(
                bound=float(np.quantile(vals, CENSOR_QUANTILE)),
                n=int(len(vals)),
                source=f"q{int(CENSOR_QUANTILE * 100)}",
                median=float(np.median(vals)),
                frac_below_top=float((vals < TOP_CONC_PIC50).mean()),
            )
    if verbose:
        print(pd.DataFrame(bounds).T.to_string())
    return bounds


def build_primary_labels(
    df: pd.DataFrame, arm: tuple = ISOFORMS, bounds: dict | None = None,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Y (n x 2k), lt_mask (n x 2k), per-compound weights (n,) for one task arm.

    ``arm`` is the tuple of isoforms this model covers. Columns 0..k-1 are the pIC50
    heads, columns k..2k-1 the matching single-dose log2fc heads.

    pIC50 columns hold real DRC values, plus — when CENSOR_NONHITS is on and ``bounds``
    is supplied — a censored bound for compounds that were screened, came back a clear
    non-hit and never reached a DRC. Those rows carry lt_mask = True, so BoundedMSE
    charges nothing for predicting anywhere below the bound. Still no pseudo-pIC50: a
    censored label asserts an inequality, not a value.
    """
    n = len(df)
    k = len(arm)
    y = np.full((n, 2 * k), np.nan)
    lt = np.zeros((n, 2 * k), dtype=bool)
    ...
    for j, iso in enumerate(arm):
        ...
        if CENSOR_NONHITS and bounds is not None:
            is_hit = df[f"{iso}_is_hit"].to_numpy(dtype=bool)
            add = (
                ~observed                          # no DRC to contradict us
                & np.isfinite(log2fc)              # but it was screened
                & ~is_hit                          # and it did not come up a hit
                & (log2fc > CENSOR_STRICT_LOG2FC)  # with room to spare, not borderline
            )
            y[add, j] = bounds[iso]["bound"]
            lt[add, j] = True
            n_censored += add
    ...
    # chemprop's per-datapoint weight is a scalar, not a vector over tasks, so a censored
    # label cannot be down-weighted independently of a real one on the same compound.
    # Apply CENSOR_WEIGHT only to rows that are *purely* censored within this arm — no
    # measured pIC50 on any of the arm's isoforms — which is the bulk of the added set
    # and leaves every real DRC row at full weight.
    if CENSOR_NONHITS and bounds is not None:
        pure = (n_censored > 0) & (w_counts == 0)
        weights = np.where(pure, CENSOR_WEIGHT, weights)
    return y, lt, weights
...
def task_weight_vector(n_tasks: int, n_primary: int | None = None) -> list[float]:
    ...
    longer always N_ISO. Defaults to half the heads when the layout is the paired
    pIC50 + log2fc one, else to all of them.
    """
    if n_primary is None:
        n_primary = n_tasks // 2 if n_tasks % 2 == 0 else n_tasks
    if n_tasks == n_primary:
        return [1.0] * n_primary
    return [1.0] * n_primary + [AUX_TASK_WEIGHT] * (n_tasks - n_primary)
...
def build_mpnn(
    ...
    weights = task_weight_vector(n_tasks, n_primary)
    criterion = (
...
def fit_mpnn(
    ...
    mpnn = build_mpnn(
        n_descriptors, train_dset.Y.shape[1], target_scaler, n_primary=n_primary, **params
    )
    trainer = pl.Trainer(
        logger=False,
...
def choose_blend_alphas(
    truth: pd.DataFrame, primary: np.ndarray, proxy: np.ndarray,
    eval_w: np.ndarray | None = None,
) -> dict:
    ...
    ``eval_w`` reweights the OOF set to the population hit rate. Without it the sweep is
    scored on a set that is 80% actives for CYP2D6, which is exactly the population where
    a weak-end correction has nothing to contribute — the previous run's CYP2D6 sweep was
    flat to four decimals (0.9214 at alpha = 0, 0.9245 at alpha = 1) for that reason. It
    was measured where it could not matter, not shown to be useless.
    """
    rows, chosen = [], {}
    for j, iso in enumerate(ISOFORMS):
        col = f"{iso}_pIC50_direct_inhibition"
        ...
        lo = np.where(np.isfinite(lo), lo, yv)
        hi = np.where(np.isfinite(hi), hi, yv)
        wv = None if eval_w is None else eval_w[m, j]

        best_alpha, best_score = 0.0, np.inf
        for alpha in BLEND_ALPHAS:
            blended = blend_predictions(primary[m, j], proxy[m, j], alpha)
            score = (
                st_rae(yv, blended, lo, hi) if wv is None
                else st_rae_weighted(yv, blended, lo, hi, wv)
            )
...
def run_primary_fold(
    ...
    Censoring bounds are fitted inside the training fold only, so a hold-out compound
    never contributes to the bound it is later scored against.
    """
    tr, va = folds != fold, folds == fold
    bounds = fit_censoring_bounds(df[tr]) if CENSOR_NONHITS else None
    y, lt, w = build_primary_labels(df, arm=arm, bounds=bounds)
    k = len(arm)
    ...
    # The hold-out drives early stopping, so it must reflect the target distribution
    # rather than the censored padding: keep only real DRC labels in the pIC50 columns.
    y_va, lt_va = y[va].copy(), lt[va].copy()
    if CENSOR_NONHITS:
        real = np.isfinite(
            np.column_stack(
                [df[f"{iso}_pIC50_direct_inhibition"].to_numpy(dtype=float)[va] for iso in arm]
            )
        )
        y_va[:, :k] = np.where(real, y_va[:, :k], np.nan)
        lt_va[:, :k] &= real
    val_dset = make_dataset(mols_va, y_va, w[va], x_va, lt_va)

    mpnn, trainer, tracker = fit_mpnn(
        train_dset, val_dset, x_tr.shape[1], params,
        max_epochs=CV_MAX_EPOCHS, patience=CV_PATIENCE, seed=seed, n_primary=k,
    )
...
def run_primary_fold_all_arms(
    ...
    for arm in TASK_ARMS:
        preds, loss, epoch = run_primary_fold(
            fold, folds, mols, df, x_d_raw, params, seed=seed, arm=arm
        )
...
def grid_search(folds, mols, df, x_d_raw, eval_w=None) -> dict:
    ...
    for i, params in enumerate(combos, start=1):
        ...
        for fold in range(N_FOLDS):
            preds, loss, epoch = run_primary_fold_all_arms(
                fold, folds, mols, df, x_d_raw, params
            )
            oof[folds == fold] = preds
            losses.append(loss)
            epochs.append(epoch)
            print(f"  fold {fold}: val_loss={loss:.4f}  best_epoch={epoch}")
        scored = score_per_isoform(df, oof, label="OOF", eval_w=eval_w)
        results.append({
            **params,
...
def main() -> None:
    ...
    # Evaluation weights that pull the DRC-labelled OOF set back to the population hit
    # rate the dense test set follows. Computed once on the full table; they depend only
    # on screening outcomes, never on predictions.
    eval_w = population_eval_weights(df) if EVAL_REWEIGHT else None
    if eval_w is not None:
        print("\n  OOF evaluation reweighted to population hit rate "
              "(hit / non-hit multipliers per isoform)")
        for j, iso in enumerate(ISOFORMS):
            lab = np.isfinite(df[f"{iso}_pIC50_direct_inhibition"].to_numpy(dtype=float))
            hit = df[f"{iso}_is_hit"].to_numpy(dtype=bool)
            wh = eval_w[lab & hit, j]
            wn = eval_w[lab & ~hit, j]
            print(f"    {iso}: hit x{wh.mean() if len(wh) else float('nan'):.3f}  "
                  f"non-hit x{wn.mean() if len(wn) else float('nan'):.3f}")

    if RUN_GRID_SEARCH:
        print(f"\n{_BANNER}\nStep 4b — grid search\n{_BANNER}")
        best_params = coerce_params(grid_search(folds, mols, df, x_d_raw, eval_w=eval_w))
    else:
    ...
    print("\nOut-of-fold scores")
    # Report both: the reweighted score is the honest estimate of leaderboard
    # performance, the raw one is comparable with every earlier run in the write-ups.
    scored_primary_raw = score_per_isoform(
        df, oof_primary, label="P: primary (raw, DRC subset)"
    )
    scored_primary = score_per_isoform(
        df, oof_primary, label="P: primary (reweighted)", eval_w=eval_w
    )
    if TRAIN_PROXY_MODEL:
        score_per_isoform(df, oof_proxy, label="X: proxy labels only", eval_w=eval_w)
        print("\nBlend sweep (ST-RAE, lower is better)")
        alphas = choose_blend_alphas(df, oof_primary, oof_proxy, eval_w=eval_w)
        oof_blend = apply_blend(oof_primary, oof_proxy, alphas)
        score_per_isoform(df, oof_blend, label="P+X: routed blend (raw)")
        scored_blend = score_per_isoform(
            df, oof_blend, label="P+X: routed blend (reweighted)", eval_w=eval_w
        )
    else:
    ...
    scores = pd.concat([
        scored_primary_raw["per_isoform"].assign(arm="primary_raw"),
    ...
    # from the folds. mean best_epoch, scaled for the extra data a full-data refit sees.
    ...
    final_bounds = fit_censoring_bounds(df, verbose=True) if CENSOR_NONHITS else None
    primary_test_preds = []
    for i, seed in enumerate(PRIMARY_SEEDS, start=1):
        ...
        for arm in TASK_ARMS:
            y, lt, w = build_primary_labels(df, arm=arm, bounds=final_bounds)
            k = len(arm)
